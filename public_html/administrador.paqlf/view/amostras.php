...
<head>
	...
	<style type="text/css">
		...
		.listagem_amostra .cabecalho span b{ font-size: 15px !important; display: block; }


		.listagem_amostra .cabecalho p{ color: #cfcfcf; margin:0; padding: 7px; text-align: center; font-size: 13px !important; }
		...
		$(document).ready(function()
		{
			...
			$(window).scroll(function() {
				...
				}else
				{
					...
					$('.listagem_amostra .cabecalho span').css({'position':'absolute', 'top':'230px', 'margin-top': '-15px'});
...
<div class="body">
	...
	<h2 class="titulo">
		...
		<form class="filtro form_amostras">
			Exercício &nbsp;&nbsp;
			<input type="text" name="data_inicial" maxlength="4" class="data" placeholder="Ano Inicial" value="<?php print $row_ano['number_year']; ?>" autocomplete="off"> -
			<input type="text" name="data_final" maxlength="4" class="data" placeholder="Ano Final" value="<?php print $row_ano['number_year']; ?>" autocomplete="off">
			<input type="hidden" name="laboratorio" value="<?php print $pk_laboratorio; ?>">
			<a href="javascript:void(0);" class="pesquisar"></a>
		</form>

		<?php print ($pk_laboratorio != '-10' && $pk_laboratorio != '' ? '<a href="/laboratorio" class="voltar"></a>' : ''); ?>
	</h2>
	...
	<table cellspacing="1" cellpadding="0" border="0" class="listagem_amostra" >	
			<input type="hidden" name="laboratorio" class="link_laboratorio" value="<?php print $pk_laboratorio; ?>">	
