...

{
	...
	$('select[name=ocultar_laboratorios]').change(function(event) {
		//alert($('.listagem_laboratorio .cabecalho td:nth-child(3) .pointer').trigger('click'));

		$('.listagem_laboratorio .cabecalho td:nth-child(3) .pointer').attr('data-rel','ASC');
		$('.listagem_laboratorio .cabecalho td:nth-child(3) .pointer').trigger('click');
	});



	$('.listagem_laboratorio .cabecalho .pointer').click(function(event)

	{
		...
		$.ajax({
			...
			data: { 'order' : param+order, 'status' : $('select[name=ocultar_laboratorios]').val() },
...

			{
				...
				$('.listagem_laboratorio .excluir').click(function(event)
	...
	$('.form_amostras .pesquisar').click(function(event)
	...
	$('.tabela_avaliacoes .cabecalho .pointer').click(function(event)
	...
	$('.form_avaliacoes .pesquisar').click(function(event)
	...
	$('.lista_contratos .excluir').click(function(event)
	...
	$(".list_programa_selo a.excluir").click(function(event) {
		/* Act on the event */
		if(confirm('Tem certeza que gostaria de excluir este selo?'))
		{
			...
			$.ajax({
				...
				url:'/modal/programa_selo_excluir.php',
	...
	$('.programa_selo_alterar button').click(function(event) {
		/* Act on the event */
		...
		$.ajax({
			...
			url:'/modal/programa_selo_alterar.php',

			type:'POST',

			data: $('.programa_selo_alterar').serializeArray(),
...

			{
...

				{
					...
					location.href="/programa/selo/" + $('.programa_selo_alterar input[name=return]').val();
	...
	$('.programa_novo_selo button').click(function(event) {
		/* Act on the event */
		...
		$.ajax({
			...
			url:'/modal/programa_selo_cad.php',

			type:'POST',

			data: $('.programa_novo_selo').serializeArray(),
...

			{
...

				{
					...
					location.href="/programa/selo/" + $('.programa_novo_selo input[name=samp]').val();
	...
	$('.alterar_selos_laboratorio button').click(function(event)
	...
	$('.list_programas input:checkbox').change(function(event)
	{
		var checked = 'false';
		var text = 'Deseja ocultar este programa?';
		if( $(this).attr('checked') == 'checked' )
		{
			checked = 'true';
			text = 'Deseja publicar este programa?';
		}
		...
		if( confirm(text) )
		{
			$.ajax({
				dataType: 'json',
				url: '/modal/programa_programa_visible.php',
				type: 'GET',
				...
				success: function( data )
				{
					if( data.rows == 1 )
					{
						if( checked == 'true' )
						{
							alerta( 'Programa Publicado com Sucesso.')	
						}else
						{
							alerta( 'Programa Publicado com Sucesso.')	
						}
	...
	$('.list_programas .calcule').click(function(event)
	{
		var div = $(this);
		if( ! $(this).hasClass('readonly') )
		{
			//var r = 
			if( confirm('Deseja Calcular Este Programa?') ) 
			{
				$.ajax({
					dataType:'json',
					url:'/modal/programa_programa_fim.php',
					type:'POST',
					...
					success: function( data )
					{
						if( data.error == 1 )
						{
							alerta('Programa calculado com sucesso.');
							div.addClass('readonly');

						}else if( data.error == 2 )
						{
							alerta('Etapas em aberto.');

						}else if( data.error == 3 )
						{
							alerta('Quantidade de amostras diferente de 12.');
	...
	$('.list_programas .excluir').click(function(event)
	...
	$('.listagem_amostra_etapa tbody .abrir').click(function(event)
	...
	$('.list_programas_etapa input:checkbox').change(function(event)

	{
		...
		var text = 'Deseja ocultar esta etapa?';
...

		{
			...
			text = 'Deseja publicar esta etapa?';
...

				{
...

					{
...

						{
							...
							alerta( 'Etapa Publicada com Sucesso.')	

						}else

						{

							alerta( 'Etapa Ocultada com Sucesso.')	
	...
	$('.list_programas_etapa .edit.readonly').click(function(event)
	...
	$('.list_programas_etapa .calcule').click(function(event)
	...
	$('.list_programas_etapa .excluir').click(function(event)
	...
	$('.list_programa_amostras .readonly').click(function(event)
	...
	/*$('.list_programa_amostras #check_all').click(function(event) {
		if( $(this).attr('checked') == 'checked' )
		{
	...
	$('.exportar_csv').click(function(event) {
		...
		$('.check_:checked').each(function(index, element) 
		{
			i = true;
		...
		if( i == false )
		{
			alert('Selecione pelo menos uma amostra');
			return true;
		}

		window.location.href ='http://'+window.location.hostname+'/modal/export_csv.php?year_samp='+checks;
	});
	...
	$('.list_programa_amostras .excluir').click(function(event)
	...
	$('.lista_controle .excluir').click(function(event)
	...
	$('.lista_contatos .excluir').click(function(event)
	...
	$('.gerar_selos').click(function(event)
