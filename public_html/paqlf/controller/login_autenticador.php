...
class AutenticadorEmBanco extends Autenticador {
    ...
    public function logar($email, $senha) {
        ...
        $sql = "SELECT *, ep_fc_people_program_active( r_pk_person ) as active

                    FROM 

                        ep_fc_validate_login_user('{$email}', '{$senha}', '{$ip}', '{$navegador}', '{$so}',false);";
        ...
        if ($dados[0]['r_login_ok'] == 't' || $dados[0]['r_login_ok'] == 'true') {
            ...
            $usuario->setLaboratorioAtivo( $dados[0]['active'] );
