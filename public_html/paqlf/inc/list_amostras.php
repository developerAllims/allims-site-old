<?php 
	...
	$query_corpo = "SELECT
			    ...
			    ,ep_program_year_steps.is_visible
			...
			WHERE
			        ep_program_year.number_year between " . $row_ano['number_year'] . " and " . $row_ano['number_year'] . "
			ORDER BY
	...
	if( $error_code == '' && $error_code_corpo == '' )
	{
		...
		print '<tbody>';
			...
			{
				print '<tr class="abrir ' . ( ($array_corpo['is_calculed'] == 't' || $array_corpo['is_calculed'] == 'true') && ( $array_corpo['is_visible'] == 't' || $array_corpo['is_visible'] == 'true' ) ? 'calculed' : 'not_calculed' ) . ' " data-id="' . $array_corpo['pk_program_year_samp'] .'">';
					...
					}else
					{
						$query_valores = "SELECT    
						             pk_pa_det_res
						            ,ep_fc_get_res_from_samp ( {$array_corpo['pk_people_samp']}, ep_pa_det_res.pk_pa_det_res, ep_pa_det_res.decimals_min_max, ep_fc_get_pk_unity_for_det_res ( {$user->getLaboratorio()} , pk_pa_det_res ), '{$array_corpo['is_visible']}' ) as resultado
						            ,norder
						        FROM
						            ep_pa_det_res
						        WHERE
